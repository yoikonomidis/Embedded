...
void at_watchdog_enable(void){
  ...
  write_node_config_reg(adc_tile, XS1_SU_CFG_WDOG_DISABLE_NUM, write_val);
}
...
void at_watchdog_set_timeout(unsigned short milliseconds){
  ...
  write_node_config_reg(adc_tile, XS1_SU_CFG_WDOG_TMR_NUM, write_val); //write expiry value
}

unsigned short at_watchdog_kick(void){
  unsigned short expiry_val, wdt_timer;
  unsigned int write_val, read_val;
  read_node_config_reg(adc_tile, XS1_SU_CFG_WDOG_TMR_NUM, read_val);  //Get current expiry value & timer
  ...
  write_node_config_reg(adc_tile, XS1_SU_CFG_WDOG_TMR_NUM, write_val);//rewrite expiry value to reset timer
